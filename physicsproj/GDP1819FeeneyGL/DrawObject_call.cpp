...
//			glPolygonMode( GL_FRONT_AND_BACK , GL_LINE );	//GL_FILL
	...
	if (pCurrentMesh->friendlyName == "PlayerShip")
	{
		std::vector<glm::vec4> contactPoints;
		// From MeshLab, the nose is about 0,0,0.4705
		glm::vec4 noseContactPoint_ModelSpace = glm::vec4(0.0f, -0.5f, 10.0f, 1.0f);
		// The wings are on the XZ plane, about -0.3 back from the centre
		glm::vec4 body1ContactPoint_ModelSpace = glm::vec4(0.0f, -0.1f, -0.3f, 1.0f);
		glm::vec4 tailTipContactPoint_ModelSpace = glm::vec4(0.0f, 0.0f, -10.5f, 1.0f);
		glm::vec4 thrust1ContactPoint_ModelSpace = glm::vec4(0.0f, 3.0f, -8.5f, 1.0f);
		glm::vec4 thrust2ContactPoint_ModelSpace = glm::vec4(2.2f, -1.5f, -8.5f, 1.0f);
		glm::vec4 thrust3ContactPoint_ModelSpace = glm::vec4(-2.2f, -1.5f, -8.5f, 1.0f);

		contactPoints.push_back(noseContactPoint_ModelSpace);
		contactPoints.push_back(body1ContactPoint_ModelSpace);
		contactPoints.push_back(tailTipContactPoint_ModelSpace);
		contactPoints.push_back(thrust1ContactPoint_ModelSpace);
		contactPoints.push_back(thrust2ContactPoint_ModelSpace);
		contactPoints.push_back(thrust3ContactPoint_ModelSpace);

		for (int i = 0; i < contactPoints.size() && i < pCurrentMesh->childObjects.size(); i++)
		{
			// Adjust the points based on orientation and position of the Mig-29:
			glm::vec4 contactPoint = glm::vec4(pCurrentMesh->childObjects[i]->position, 1.0f);
			...
			contactPoint_WorldSpace = matTransform * contactPoints[i];
			...
			cMeshObject* pDebugBall = pCurrentMesh->childObjects[i];
			...
			pDebugBall->position = contactPoint_WorldSpace;

			if (pDebugBall->bIsVisible)
			{
