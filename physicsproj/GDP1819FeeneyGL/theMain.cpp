...
std::vector<cParticleEmitter*> gAPEs;
...
//	float HACK_HEIGHT_MAP_RATIO_ADJUST = 1.0f;
	...
	//
	cMeshObject * playerShip = findObjectByFriendlyName("PlayerShip");

	for (int i = 0; i < playerShip->childObjects.size(); i++)
	{
		cParticleEmitter * gAPE = new cParticleEmitter();		//"A Particle Emitter"
		// This takes a while
		gAPE->Init(5.0f, 5.0f,
			500,
			playerShip->childObjects[i]->position,
		...
		gAPEs.push_back(gAPE);
	}

	//cParticleEmitter * gAPE = gAPEs[0];		//"A Particle Emitter"
...
//		if ( ! bButtonPressed )
		{
			...
			{
				gAPEs[0]->UpdatePosition(playerShip->childObjects[0]->position);
				// This is fast
				gAPEs[0]->Reset();
				bButtonPressed = true;
...
//			glUniform1f( bAddReflect_UniLoc, (float)GL_TRUE );
		...
		for (int i = 0; i < gAPEs.size(); i++)
		{
			if (gAPEs[i]->active)
			{
				...
				gAPEs[i]->Update(deltaTime);

				unsigned int numParticles = (unsigned int)gAPEs[i]->vecParticles.size();
				unsigned int count = 0;
				for (unsigned int index = 0; index != numParticles; index++)
				{
					if (gAPEs[i]->vecParticles[index].lifeRemaining > 0.0f)
					{
						// Draw it
						pParticleMesh->position = gAPEs[i]->vecParticles[index].position;
...
//		sceneCommandGroup.Update(deltaTime);
		...
		/*cMeshObject* pCar = findObjectByFriendlyName("car");
		glm::quat test = pCar->getQOrientation();
		std::cout << "car rots: " << test.x << " " << test.y << " " << test.z << std::endl;*/
		...
		ssMoveTo << "r, x, y, z, vx, vy, vz, movetimer, rotx, roty, rotz = getObjectState(";
		ssMoveTo << ::g_pObjectToRotate->getID();
		ssMoveTo << ") \n";

		ssMoveTo << "if movetimer > 0 then y = y + 0.1 end \n";
		ssMoveTo << "movetimer = movetimer - 1.0  \n";
		//ssMoveTo << "print(movetimer)  \n";
		ssMoveTo << "moveto(";
		ssMoveTo << ::g_pObjectToRotate->getID();
		ssMoveTo << ", x, y, z, Vx, Vy, Vz, movetimer ) \n";

		ssMoveTo << "if roty > -90 then roty = roty - 0.1 end \n";
		ssMoveTo << "print(roty)  \n";
		ssMoveTo << "rotateto(";
		ssMoveTo << ::g_pObjectToRotate->getID();
		ssMoveTo << ", x, y, z, Vx, Vy, Vz, movetimer ) \n";
...
//#endif 
	...
	{// START OF: AABB debug stuff
		...
		cMeshObject* pTheBunny = findObjectByFriendlyName("PlayerShip")->childObjects[0];
		...
		//glm::vec3 maxXYZ = pTheBunny->position + pTheBunny->nonUniformScale.x;
		unsigned long long ID_of_AABB_We_are_in = cAABB::generateID( pTheBunny->position, maxXYZ );
		...
		for ( ; itAABB !=  ::g_pTheTerrain->m_mapAABBs.end(); itAABB++ )
		{
			...
			cubeCorners[6] = pCurrentAABB->getMinXYZ();
			cubeCorners[7] = pCurrentAABB->getMinXYZ();
			...
			cubeCorners[6].x = pCurrentAABB->getMaxXYZ().x;
			cubeCorners[6].z = pCurrentAABB->getMaxXYZ().z;

			cubeCorners[7].y = pCurrentAABB->getMaxXYZ().y;
			cubeCorners[7].z = pCurrentAABB->getMaxXYZ().z;

			// Draw line from minXYZ to maxXYZ
			if (mode == 2)
			{
				::g_pDebugRenderer->addLine( cubeCorners[0], cubeCorners[1], glm::vec3(1,1,1), 0.0f );
				/*::g_pDebugRenderer->addLine( cubeCorners[0], cubeCorners[2], glm::vec3(1,1,1), 0.0f );
				::g_pDebugRenderer->addLine( cubeCorners[0], cubeCorners[3], glm::vec3(1,1,1), 0.0f );
				::g_pDebugRenderer->addLine( cubeCorners[0], cubeCorners[4], glm::vec3(1,1,1), 0.0f );

				::g_pDebugRenderer->addLine( cubeCorners[1], cubeCorners[5], glm::vec3(1,1,1), 0.0f );
				::g_pDebugRenderer->addLine( cubeCorners[1], cubeCorners[6], glm::vec3(1,1,1), 0.0f );
				::g_pDebugRenderer->addLine( cubeCorners[1], cubeCorners[7], glm::vec3(1,1,1), 0.0f );*/
			}
		}
	...
	// ********************************************************************************
		...
		//theTerrainMesh.meshFileName = "mountainterrain.ply";		// "MeshLab_Fractal_Terrain_xyz.ply";
...
void LoadTerrainAABB(void)
{
	...
	for ( unsigned int triIndex = 0; triIndex != terrainMeshInfo.numberOfTriangles; triIndex++ )
	{
		...
		{
			glm::vec3 maxXYZ = curAABBTri.verts[0] + sideLength;
